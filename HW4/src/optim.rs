...
#[test]
fn lhs() {
    let P: DMatrix<f64> = DMatrix::from_vec(
        2, 2, vec![1.0, 2.0, 3.0, 4.0]);
    let A: DMatrix<f64> = DMatrix::from_vec(
        3, 2, vec![5.0, 6.0, 7.0, 8.0, 9.0, 10.0]);
    let expected: DMatrix<f64> = DMatrix::from_vec(5, 5, vec![
        1.0, 2.0, 5.0, 6.0, 7.0, 3.0, 4.0, 8.0, 9.0, 10.0, 5.0, 8.0, 0.0, 0.0,
        0.0, 6.0, 9.0, 0.0, 0.0, 0.0, 7.0, 10.0, 0.0, 0.0, 0.0
    ]);
